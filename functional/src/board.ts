...
export function canMove<T>(board: Board<T>, first: Position, second: Position): boolean {
  ...
  const newBoard = swapPieces(board, first, second);
  return matchesExists(newBoard, first, newBoard.pieces[first.row][first.col]) && !matchesExists(newBoard, second, newBoard.pieces[second.row][second.col]);
}
...
export function move<T>(generator: Generator<T>, board: Board<T>, first: Position, second: Position): MoveResult<T> {
  ...
  return {
    board: newBoard,
    effects: handleCascadeEffect(generator, newBoard, [])
  }
}
